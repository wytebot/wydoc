...
export default async function handler(req,res){
  ...
  try{
    ...
    if(action==='init'){
      ...
      const token=await googleAccessToken(form.owner_id); const root=await ensureFolder(token,'WyDoc Attachments'); const folder= form.drive_folder_id || await ensureFolder(token,form.name.slice(0,80),root);
